...
exports.createDiagramsMessage = void 0;
const utils_1 = require("./utils");
const format24DateTime = (date) => {
    ...
    const numToZeroPadStr = (num, len = 2, fillStr = ' ') => num.toString().padStart(len, fillStr);
...
// メイン
const createMainContent = (diagram) => ({
    type: 'box',
    layout: 'vertical',
    contents: [
        {
        ...
        },
        {
            ...
            contents: [
                {
                    type: 'text',
                    text: diagram.arrivals.length === 0
                        ? '本日の運行は終了しています'
                        : diagram.arrivals
                            .slice(0, 3) // 直近3件のみ表示
...
// フッタ
const createFooterFlexBox = (data) => ({
    type: 'box',
    layout: 'vertical',
    contents: [
        {
...
// バブルコンテンツ
const createFlexBubbleContent = (data) => ({
    type: 'bubble',
    size: 'kilo',
...
const createDiagramsMessage = (messageDataList) => ({
    type: 'flex',
    altText: messageDataList.length === 1 ? `${messageDataList[0].station.officialName}からの地下鉄発車時刻を送信しました` : '地下鉄発車時刻を送信しました',
    contents: {
        type: 'carousel',
        contents: messageDataList.map((data) => createFlexBubbleContent(data)),
    },
});
exports.createDiagramsMessage = createDiagramsMessage;
