...
test("specifyStationsByNameメソッド", () => {
  expect(stations.specifyStationsByName("").map((s) => s.officialName)).toEqual(
    []
  );
  expect(
    stations.specifyStationsByName("えき").map((s) => s.officialName)
  ).toEqual([]);
  expect(
    stations.specifyStationsByName("駅").map((s) => s.officialName)
  ).toEqual([]);
  expect(
    stations.specifyStationsByName("エキ").map((s) => s.officialName)
  ).toEqual([]);
  expect(
    stations.specifyStationsByName("さ").map((s) => s.officialName)
  ).toEqual([]);
  expect(
    stations.specifyStationsByName("sapporo").map((s) => s.officialName)
  ).toEqual([]);

  expect(
    stations.specifyStationsByName("さっぽろ駅").map((s) => s.officialName)
  ).toEqual(["さっぽろ駅", "さっぽろ駅"]);
  expect(
    stations.specifyStationsByName("札幌").map((s) => s.officialName)
  ).toEqual(["さっぽろ駅", "さっぽろ駅"]);
  expect(
    stations.specifyStationsByName("さっぽろ").map((s) => s.officialName)
  ).toEqual(["さっぽろ駅", "さっぽろ駅"]);
  expect(
    stations.specifyStationsByName("サッポロ").map((s) => s.officialName)
  ).toEqual(["さっぽろ駅", "さっぽろ駅"]);
  expect(
    stations.specifyStationsByName("ｻｯﾎﾟﾛ").map((s) => s.officialName)
  ).toEqual(["さっぽろ駅", "さっぽろ駅"]);
